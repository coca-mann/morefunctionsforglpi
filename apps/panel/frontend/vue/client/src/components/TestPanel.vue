<template>
  <div v-if="visible" class="fixed bottom-4 right-4 w-96 bg-slate-800 border-2 border-purple-500 rounded-lg p-6 shadow-2xl z-40 max-h-96 overflow-y-auto">
    <!-- Header -->
    ...
    <div class="space-y-4">
      ...
      <div class="pt-4 border-t border-slate-700">
        ...
        <div class="space-y-2">
          <input
            v-model="projectUpdateTitle"
            type="text"
            placeholder="Nome do projeto"
            class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-purple-500"
          />
          <input
            v-model.number="projectProgress"
            type="number"
            min="0"
            max="100"
          ...
          />
          <button
            @click="sendProjectUpdate"
            :disabled="!projectUpdateTitle"
